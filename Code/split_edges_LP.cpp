...
int main(int argc,  char **argv){
  ...
  ofstream out_config_file(("./DY_LP_Dataset/" + batch_queryname + "/config.txt").c_str());
  ...
  while(increment_round_index < snapshots_number && count_edges != NUMBER_IN_A_GROUP * batch_update_snapshots_number){
    ...
    while (infile.good()) {
        ...
        if(count_edges % NUMBER_IN_A_GROUP == 0){
            ...
            string current_edge_file_name = "./DY_LP_Dataset/" + batch_queryname + "/" + batch_queryname + "-Train-" + to_string(round_index) 
                + "-" + to_string(num) + ".txt";
